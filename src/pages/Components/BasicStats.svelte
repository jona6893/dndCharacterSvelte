...
function updateBasicStats(attribute) {
  ...
  if(attribute.target.type === "checkbox"){
...
$: if ($currentCharacter && $currentCharacter.basicStats) {
  inspiration = $currentCharacter.basicStats.inspiration;
}

</script>

<div class="row-start-1 flex flex-col h-full w-full items-center justify-around gap-4 ">
    ...
    <div class="bg-gray-100 shadow-md flex flex-col justify-center w-28 items-center gap-2 p-4 ">
        ...
        <input value={$currentCharacter?.basicStats?.speed ? $currentCharacter?.basicStats?.speed : 0} on:change={(e) =>updateBasicStats(e)} class="w-8 bg-transparent border-b border-gray-400 text-lg text-center" type="text" name="speed" id="">
        <span class="text-xs">SPEED</span>
    </div>
     <!-- ARMOR CLASS -->
    <div class="bg-gray-100 shadow-md flex flex-col justify-center w-28 items-center gap-2 p-4 ">
        <span class="text-xs">ARMOR</span>
        <input value={$currentCharacter?.basicStats?.armor ? $currentCharacter?.basicStats?.armor : 0} on:change={(e) =>updateBasicStats(e)} class="w-8 bg-transparent border-b border-gray-400 text-lg text-center" type="text" name="armor" id="">
        <span class="text-xs">CLASS</span>
    </div>
</div>
<div class="flex justify-between gap-4 col-span-2">
    <!-- INITIATIVE -->
    <div class="bg-gray-100 shadow-md flex flex-col justify-center w-20 items-center gap-2 p-2 py-4">
       
        <input value={$currentCharacter?.basicStats?.initiative ? $currentCharacter?.basicStats?.initiative : 0} on:change={(e) =>updateBasicStats(e)} class="w-8 bg-transparent border-b border-gray-400 text-lg text-center" type="text" name="initiative" id="">
    ...
    <div class="bg-gray-100 shadow-md flex flex-col justify-center w-28 items-center gap-2 p-2 py-4 h-full">
        ...
        <input value={$currentCharacter?.basicStats?.proficiency ? $currentCharacter?.basicStats?.proficiency : 0} on:change={(e) =>updateBasicStats(e)} class="w-8 bg-transparent border-b border-gray-400 text-lg text-center" type="text" name="proficiency" id="">
        <span class="text-xs">BONUS</span>
    </div>
    <!-- INSPIRATION -->
    <div class="bg-gray-100 shadow-md flex w-20 flex-col justify-center items-center gap-2 p-2 py-4 relative">
        <input bind:checked={inspiration}  on:change={(e) => updateBasicStats(e)} class="inspiration z-[1] w-8 bg-transparent border-b border-gray-400 text-lg text-center" type="checkbox" name="inspiration">
